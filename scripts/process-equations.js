...
module.exports = {
	processEquations: function () {
		...
		for (let file of equationFiles) {
			...
			let equations;
			if (equation.indexOf('\n') !== -1) {
				// tex file may contain multiple equations. If so, split them 
			...
			for (let eq of equations) {
				mjAPI.typeset({
					...
					if (!data.errors) {
						// for multiple equations, use counter like _01.svg, _02.svg, _03.svg,...
						utils.strToFile(file.replace('equations', 'equations_generated')
							.replace('.tex', equations.length === 1 ? '.svg' : '_' + pad(2, counter++) + '.svg'), data.svg);
					}
				});
			}
		}
